...
# Create a security group for the app node (SSH, HTTP, and port 3000)
resource "aws_security_group" "app_node_sg" {
  name        = "app-node-sg"
  description = "Allow SSH, HTTP, and Node.js app port"
...
resource "aws_instance" "app_node" {
  ...
  tags = {
    Name = "tech508-naseem-ubuntu-2204-ansible-target-node-app" 
  }
}
